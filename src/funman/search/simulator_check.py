...
class SimulatorCheck(Search):
    ...
    def eval_point(
        ...
        # parameters
        def beta(t):
            return np.piecewise(t, [t >= 0], [beta_val])

        def gamma(t):
            return np.piecewise(t, [t >= 0], [gamma_val])
        ...
        query = (
            "1"
            if query_condition(sim, tstart, tend, dt, beta_val, gamma_val)
            else "0"
        )
        param_assignments = {
            "beta": beta_val,
            "gamma": gamma_val,
            "assignment": query,
        }  # for "all", go through every option. for "any", only need one good parameter choice.
    ...
    def ps(
        self,
        parameter_space,
        num_dim_points=10,  # parameter_search_bounds
        # , query_condition=query_1, rtol=1e-3, #num_dim_points=10, plot=False
    ):
        parameters = (
            parameter_space.model_parameters()
        )  # list(parameter_search_bounds.keys())
        parameter_points = {
            p: np.linspace(
                parameter_space.bounds[p].lb,
                parameter_space.bounds[p].ub,
                num_dim_points,
            )
            for p in parameters
        }
        ...
        for i, point in enumerate(points):
            ...
            if param_assignments["assignment"] == "1":
    ...
    def expand(
        ...
        for gridpoint in gridpoints:
            ...
            with Solver(
                ...
                formula, simplified_formula, model_formula = (
                    self.build_formula(episode, schedule, options)
                )
                    ...
                    if result is not None and isinstance(result, pysmtModel):
                        ...
                        result_assignment = And(
                            ...
                            + [
                                ...
                                if p.is_unbound()
                                and p.name not in assigned_vars
                            ]
                        )
