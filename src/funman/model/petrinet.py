...
from .generated_models.petrinet import Observable, State, Transition, Initial, Model1, Parameter, Rate, State,Transition, Properties, Model, Semantics, OdeSemantics, Distribution
...
class GeneratedPetriNetModel(AbstractPetriNetModel):
    ...
    def stratify(
        ...
        if strata_parameters is not None:
            ...
            src_only_parameters = [ 
                ...
                if t_id in src_only_transitions and sympy.Symbol(sp) in old_rates[t_id][0].free_symbols 
                for level in strata
            ...
            dest_only_parameters = [
                ...
                for t_id,r in old_rates.items() 
                for sp in strata_parameters
                if t_id in dest_only_transitions and sympy.Symbol(sp) in old_rates[t_id][0].free_symbols 
                for level in strata
                ]
            ...
            src_and_dest_parameters = [
                ...
                for t_id,r in old_rates.items() 
                for sp in strata_parameters
                if t_id in src_and_dest_transitions and sympy.Symbol(sp) in old_rates[t_id][0].free_symbols 
    ...
    def abstract(self, state_abstraction: Dict[str, str]):
        ...
        def aggregate_rates(self, rates, abstraction):
            ...
            abstraction_substitution = {sympy.Symbol(i_abstraction[k][0]): to_sympy(f"{k}-{'-'.join(i_abstraction[k][1:])}", self._symbols()+list(abstraction.values())) for k in i_abstraction.keys()}
