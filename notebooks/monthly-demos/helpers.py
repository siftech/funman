...
def get_model(model):
    return Runner().get_model(model) if isinstance(model, dict) else Runner().get_model(model)

def set_timepoints(funman_request, timepoints):
    if funman_request.structure_parameters is not None and len(funman_request.structure_parameters) > 0:
        funman_request.structure_parameters[0].schedules = [EncodingSchedule(timepoints=timepoints)]
    else:
        funman_request.structure_parameters =[Schedules(schedules = [EncodingSchedule(timepoints=timepoints)])]
...
def get_synthesized_vars(funman_request):
    return [p.name for p in funman_request.parameters if p.label == "all"] if funman_request.parameters is not None else []
