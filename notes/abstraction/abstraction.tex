...
\begin{example}\label{ex:abstraction}
	An abstraction of the stratified SIR model is $\Theta^A =$
	\begin{eqnarray*}
		P^A &=& \{\beta_{vac=T}, \beta_{vac=F}, \gamma\}\\
		D^A &=& \{vac\}, dom(vac) = \{T, F\}\\
		X^A &=& \{S, I, R\}\\
		G^A &=& \{(vac=T), (vac=F)\}\\
		Z^A &=& \{inf_{vac=T,vac=F}, rec\}\\
		{\cal X} &=& \left\{
		\begin{array}{ll}
			v_{S,vac=T, vac=F} & :S \\
			v_I                & :I \\
		...
		{\cal R} &=& \left\{
		\begin{array}{ll}
			\beta_{vac=T} S_{vac=T} I + \beta_{vac=F} S_{vac=F} I & : z_{\mathit{inf_{vac=T,vac=F}}} \\
...
In Example \ref{ex:abstraction}, the abstraction $(\Theta^{A}, \Omega^{A})$ maps the $(S, \{(vac=F)\})$ and $(S, \{(vac=T)\})$ state variables to the $(S, \{(vac=T), (vac=F)\})$ state variable (de-stratifying the \Petrinet{}).  In combining the state variables, the abstract \Petrinet{} consolidates the transitions $inf_{vac=F}$ and $inf_{vac=T}$ and associated rates from susceptible to infected.
