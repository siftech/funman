...
\begin{table}\centering
	...
	\end{tabular}
	\caption{\label{tab:sirhd_results}  Runtime in seconds to simulate each model formulation.  }

\end{table}

Table \ref{tab:sirhd_results} lists the runtime in seconds required to simulate each SIRHD model variation.  Each instance seeks to check a constraint that assesses whether over 200 days the number of infected is no more than 30\% of the population ($N=1.5e9$).  We use 15 stratifications on the model, three for each of the state variables $S$, $I$, $R$, $H$ and $D$.  The three stratifications involve stratifying each state variable into vaccinated and unvaccinated groups ($S_{vac}$ and $S_{unvac}$), the vaccinated group into age groups (e.g., $S_{vac,0}$, $S_{vac,1}$, $S_{vac,2}$, ...), and the unvaccinated group into age groups ($S_{unvac,0}$, $S_{unvac,1}$, $S_{unvac,2}, \ldots$).  We report results for either three or five age groups.  Model 0 in the Stratified column is the base model.  Models 1-15 in the Stratified column are successive stratifications (refinements) of the base model 0 in the Stratified column.  The bolded number in the Stratified column is the fully stratified model that is the starting point for analysis.  The bolded number in the Bounded Abstracted column is the smallest model that allows us to confirm the model constraint.  Entried in the Bounded Abstracted column are successive abstractions of the models, where model 14 is the abstraction of model 15 in the Stratified column,  model 13 is the abstraction of model 14 in the Bounded Abstracted column, model 12 is the abstraction of model 13, etc.

We construct the most abstract model 0 from a series of stratifications and abstractions.  It is an over-approximation of the most stratified model that may allow us to prove the constraint is satisfied.  The abstract bounded model represents lower and upper bounds on the number of infected, and if the upper bound is less than 30\% of $N$ (checked through simulation) then we do not need to simulate the most stratified model 15 (our reference model formulation).  The bounded abstract model 0 can be inconclusive if 30\% of $N$ falls between the lower and upper bounds on the number of infected.  Considering successively less abstract models (proceeding from model 0 to 1, 2, etc.), we expect the lower and upper bounds to tighten.  The bolded time in the Bounded Abstracted model column is the runtime of the first model formulation (starting from model 0) where we can prove that the upper bound on the number of infected is less than 30\% of $N$.  Therefore, if we need to simulate Bounded Abstracted models 0 through 3, we require 28.29  and 42.69 seconds, respectively, for either three or five age groups.  Compared to the time to simulate the largest Stratified model 15, it takes 15.17 or 82.80 seconds respectively.  The savings due to abstraction reduces runtime by one half when there are five age groups, but doubles the runtime when there are three age groups.  This illustrates the trade-off between simulating several abstract models versus one large stratified model.  When the abstract models can capture the important model dynamics using bounds and the number of collapsed stratification levels is large, then abstraction is a win.

One way to make abstraction more productive would be to estimate the size of model required to (dis)confirm a property in question.  If one could do this with some reliability, it would reduce the number of fruitless model solutions.  For example, if one could go directly from model 0 to model 3 in the three age level case, the tradeoff would improve from 28.29/15.17s (models 0-3) to 15.19/15.17s (models 0 and 3 only).  In this case that is still not enough to make the tradeoff a favorable one, but it is quite likely that in other cases it would be.

The above comparison suggests another possible improvement: if one could estimate the cost of model solution for different models, one could identify models, like Stratified Model 15, that should simply be directly solved, instead of bothering with abstraction.

\textcolor{blue}{Note from Drisana: Would it be helpful to add information about how to estimate the simulation times associated with each successive stratification?  Also (just out of curiosity) were these simulations using odeint or some other simulator? It may be helpful to include if that's possible.}
